                    ...
                    db.get('SELECT COUNT(*) FROM address',
                        function (err, data) {
                            ...
                            if (err) {
                                ...
                                console.log(data["COUNT(*)"]);
                                if (data["COUNT(*)"] == 0) {
                                    var insert = 'INSERT INTO address(city, state, country) VALUES (?,?,?)'
                                    db.run(insert, ["Nagpur", "Maharashtra", "India"])
                                    db.run(insert, ["Ahmedabad", "Gujarat", "India"])
                                    db.run(insert, ["Panaji", "Goa", "India"])
